...
; NICBEEP.ASM 
; 
; Nicam test functions
;
; Date: 20000102 JWD initial version
;-----------------------------------------------------------;
...
;	.ps     0FB00h          ; why was this? For the monitor?
	.entry                  ;
...
;	lalk    15949
	...
	lack    025h            ; Turn on XINT and INT2,0 , Rint off
	sacl    IMR             ;
...
;-------------------------------------------------------------------
; Toggle buffers (not needed in beep version?)
;-------------------------------------------------------------------
...
;-------------------------------------------------------------------
; Copy beep to the sample buffer ....
;-------------------------------------------------------------------

	lar	AR1,INBUF	; points to input buffer
	larp	AR1
	rptk	63		; nr of samples-1
	blkp	BEEPTAB,*+
...
pre_loop:                       ; (arp = AR1 now)
	...
	lac     *+,0,AR3        ; accu = current sample pointed to by AR1
	abs                     ; make absolute
	sacl    *+,0,AR5        ; and store in maxbuf, poited to by AR3
	banz    pre_loop,*-,AR1	
...
#IFDEF BOTH_CH
	.include "beeptabb.bin"
#ENDIF
#IFDEF LEFT_CH
	.include "beeptabl.bin"
#ENDIF
#IFDEF RIGHT_CH
	.include "beeptabr.bin"
#ENDIF
	.word	0
